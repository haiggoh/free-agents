...
#   remote-keys.sh --env <provider>      # print `NAME=value` lines for eval (SECRET — pipe, never log)
...
# Kept in sync with bin/remote_provider_core.py's PROVIDERS registry.
_provider_spec() {
    case "$1" in
        ...
        github)     echo "GITHUB_MODELS_TOKEN:github-models" ;;
...
ALL_PROVIDERS="gemini groq openrouter cloudflare github cerebras nvidia"
